...
def _enforce_busan_visual_contract(story_data: Dict[str, Any], req: GenerateRequest) -> Dict[str, Any]:
    ...
    # Busan competition stories use Boogi as the only named foreground character.
    # Any extra sheet generated by the LLM is discarded to avoid creating new recurring characters.
    concept["character_sheets"] = [boogi_sheet]
    ...
    if isinstance(pages, list):
        for page in pages:
            ...
            if "부기" not in image_prompt and "boogi" not in image_prompt.lower():
                image_prompt = (
                    "부산시 마스코트 부기가 장면 중심에 단독으로 등장해 부산 장소를 소개한다. "
                    f"{image_prompt}"
                ).strip()
            style_suffix = (
...
def _build_busan_context_block(req: GenerateRequest) -> str:
    ...
    fact_lines = [
        _fact_line("한줄 소개", busan_ctx.introduction or busan_ctx.subtitle, 220),
        _fact_line("주요 특징", busan_ctx.feature_summary, 260),
        _fact_line("유래/역사 포인트", busan_ctx.origin_story, 260),
        _fact_line("스토리 참고 요약", busan_ctx.description, 320),
        _fact_line("동화 소재 힌트", busan_ctx.story_seed, 260),
    ]
    fact_lines = [line for line in fact_lines if line]
    visual_lines = [
        _fact_line("관광사진 제목", busan_ctx.photo_title, 120),
        _fact_line("관광사진 촬영지", busan_ctx.photo_location, 120),
        _fact_line("관광사진 키워드", busan_ctx.photo_keywords, 180),
    ]
    visual_lines = [line for line in visual_lines if line]

    return "\n".join([
        "[공식 Fact Pack]",
        f"- 공식 장소명: {busan_ctx.title or '미상'}",
        ...
        *(visual_lines or ["- 사진 키워드 없음. 함께 제공된 장소 이미지를 우선 관찰할 것."]),
        ...
        "- 정보가 부족하면 '그곳에 이런 역사가 있었다'고 단정하지 말고, 부기가 풍경을 관찰하고 궁금해하는 방식으로 처리한다.",
        "[상상 허용]",
        "- 부기의 행동, 감정, 페이지별 모험 순서, 대화 톤은 창작해도 된다.",
        "- 단, 창작 요소와 공식 사실이 섞여 독자가 허위 사실로 이해하지 않게 한다.",
    ])


def _build_busan_story_seed_block(req: GenerateRequest) -> str:
    busan_ctx = req.busan_context
    if not busan_ctx:
        return "- 장소명과 사진 키워드를 중심으로 부기가 직접 발견하고 소개하는 장면을 만든다."
    if busan_ctx.story_seed:
        return f"- 우선 활용할 동화 소재 힌트: {_trim_fact(busan_ctx.story_seed, 320)}"
    ...
    if busan_ctx.feature_summary:
        story_seed_items.append(f"주요 특징 활용: {_trim_fact(busan_ctx.feature_summary, 180)}")
    if busan_ctx.origin_story:
        story_seed_items.append(f"유래/역사 포인트 활용: {_trim_fact(busan_ctx.origin_story, 180)}")
    if busan_ctx.photo_keywords:
        story_seed_items.append(f"관광사진 키워드를 시각 장면에 반영: {_trim_fact(busan_ctx.photo_keywords, 140)}")
    if busan_ctx.photo_title:
        story_seed_items.append(f"사진 제목에서 연상되는 장면 활용: {_trim_fact(busan_ctx.photo_title, 100)}")
    return "\n".join(f"- {item}" for item in story_seed_items) if story_seed_items else "- 장소명과 사진 키워드를 중심으로 부기가 직접 발견하고 소개하는 장면을 만든다."


def _selected_busan_place_name(req: GenerateRequest) -> str:
...
def _build_busan_prompt(req: GenerateRequest) -> str:
    ...
    busan_context_block = _build_busan_context_block(req)
    busan_story_seed_block = _build_busan_story_seed_block(req)

    mission_rules = {
        "CITY_INTRO": dedent("""
        - 부산 공식 캐릭터 '부기'가 매 페이지의 핵심 행동에 단독으로 참여한다.
        - 선택된 부산 장소를 배경으로 도시의 특징(바다·시장·다리·골목)을 자연스럽게 소개한다.
        - 홍보문처럼 나열하지 말고 부기가 직접 걷고 발견하는 사건 중심으로 구성한다.
        - 이름 있는 어린이 주인공, 새 조력자, 동물 친구를 만들지 않는다.
        """).strip(),
        "HERITAGE": dedent("""
        - 부산 공식 캐릭터 '부기'가 문화유산 탐험의 안내자 역할을 단독으로 맡는다.
        - 제공된 장소/설명 기반 사실 정보를 최소 1개 이상 포함한다.
        - 연도/고유명사 등 확신 없는 사실은 단정하지 말고 일반 설명으로 완화한다.
        - 이름 있는 어린이 주인공, 새 조력자, 동물 친구를 만들지 않는다.
        """).strip(),
        "MULTICULTURAL": dedent("""
        - 부산 공식 캐릭터 '부기'가 부산의 다문화 공간, 음식, 언어, 축제, 항구 도시의 만남을 직접 소개한다.
        - 다양한 문화권의 사람들은 필요할 때만 배경의 작은 군중으로 표현하고 이름 있는 반복 캐릭터로 만들지 않는다.
...
- 위 Fact Pack에 없는 구체적 연도, 인물, 사건, 설화, 순위, 문화재 지정 정보는 새로 지어내지 않는다.
- 역사/유래 정보가 부족하면 사실을 꾸며내지 말고, 장소의 분위기·사진 키워드·부기의 탐험 중심으로 이야기를 만든다.
- 공공데이터 설명을 그대로 복사하지 말고, 부기가 직접 보고 설명하는 행동·발견 장면으로 바꾼다.
- 공식 사실과 창작 장면이 섞여 독자가 허위 사실로 이해하지 않도록 표현한다.
...
- 부기는 부산 대표 안내 캐릭터다. 모든 page.text와 page.image_prompt에 반드시 등장시키며, 첫 페이지와 마지막 페이지도 예외로 두지 않는다.
- 이름 있는 전면 캐릭터는 부기만 사용한다. 어린이 주인공, 새 조력자, 동물 친구, 요정 같은 반복 캐릭터를 새로 만들지 않는다.
- creative_concept.character_sheets에는 부기만 포함하고, 다른 캐릭터 시트를 만들지 않는다.
- 부기를 별칭으로 다시 만들거나 같은 장면에 복제하지 않는다.
...
- 부산시 마스코트 부기는 모든 page.image_prompt에 반드시 이름으로 명시한다.
- 부기 외의 이름 있는 캐릭터를 page.text나 page.image_prompt에 만들지 않는다.
...
def generate_busan_story(req: GenerateRequest, request_id: str) -> GenerateResponse:
    ...
    for attempt in range(1, max_attempts + 1):
        try:
            ...
            place_name = _selected_busan_place_name(req)
            if str(req.language).upper() == "KO" and place_name and not _story_mentions_place(story_data, place_name):
                raise ValueError(f"Busan story must mention the selected public-data place: {place_name}")
